...
public class GiantBlockModel implements StitchBlockModel {
    ...
    public List<StitchQuad> getQuads(BlockAndTintGetter level, BlockState blockState, BlockPos pos, Direction direction) {
        ...
        return switch (direction.getAxis()) {
            case X -> {
                ...
                yield List.of(StitchQuad.withSprite(1 + (z % width) + (y % height) * height));
            }
            case Z -> {
                if (direction.getAxisDirection() == Direction.AxisDirection.NEGATIVE) {
                    x = Math.abs(x - width - 1);
                }
                yield List.of(StitchQuad.withSprite(1 + (x % width) + (y % height) * height));
            }
            default -> {
                if (direction.getAxisDirection() == Direction.AxisDirection.NEGATIVE) {
                    z = Math.abs(z - width - 1);
                }
                yield List.of(StitchQuad.withSprite(1 + (x % width) + (z % height) * height));
            }
        };
    }

...
import earth.terrarium.stitch.api.client.utils.forge.NullableEnumMap;
...
public class StitchBakedModel implements IDynamicBakedModel {
    ...
    public @NotNull ModelData getModelData(@NotNull BlockAndTintGetter level, @NotNull BlockPos pos, @NotNull BlockState state, @NotNull ModelData data) {
        ...
        for (Direction direction : DIRECTIONS) {
            List<StitchQuad> culledQuads = new ArrayList<>();
            List<StitchQuad> unculledQuads = new ArrayList<>();
            for (StitchQuad quad : this.model.getQuads(level, state, pos, direction)) {
                if (quad.depth() == 0f) {
    ...
    private List<BakedQuad> bakeQuads(List<StitchQuad> quads, Direction direction) {
        ...
        for (StitchQuad quad : quads) {
            bakedQuads.addAll(ForgeStitchUtils.bakeQuad(quad, direction, this.textures.get(quad.sprite())));
        }
        return bakedQuads;
    }
